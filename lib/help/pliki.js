...
class DocumentClass {
	...
	writeFile(document, contents) {
		fs.writeFile(document, contents, (err) => {
			if (err) {
				console.error(
					`Couldn't save ${document} file with contents:${contents} : ${err} `
				);
			}
		});
	}
	...
	appendFileSync(document, contents) {
		fs.appendFileSync(document, `\n${contents.trimEnd()}`, (err) => {
			if (err) {
				console.error(
					`Couldn't append ${document} file with contents:${contents} : ${err} `
				);
			}
		});
	}
